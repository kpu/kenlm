...
void DisplayHelp(const char *name) {
  std::cerr
		<< "Usage: " << name << " type input.arpa output.arpa\n"
		   "If type is single, then a single vocabulary is gathered from stdin and used to filter\n"
			 "If type is multiple, then a separate vocabulary is computed from each line.  A separately filtered file is created for each line's vocabulary, named by appending the line number.\n"
			 "If type is union, then a single model is created containing the union of line-level models that would be generated by multiple.\n";
}
