...
template <class Callback> void JointOrder(const ChainPositions &positions, Callback &callback) {
  ...
  while (true) {
    ...
    if (!memcmp(streams[static_cast<int>(current) - 1]->begin(), streams[current]->begin() + 1, sizeof(WordIndex) * current)) {
